...
# path = 'F:\\datasets\\stationFlow\\allstation'
# dateparse = lambda dates: pd.datetime.strptime(dates, '%Y-%m-%d %H:%M:%S')
...
#---------------------------------------------------------
df = pd.read_csv('F:\\datasets\\stationFlow\\midResult',names=['time','name','inflow','outflow','year','month','day','hour','weekday'])
...
df = df[(df['label']==0)&(df['month']<=7)&(df['month']!=3)] # 上半年非工作日
##站点
station = df[df["name"] == "世界之窗"]
station.to_csv("F:\\datasets\\stationFlow\\sjzc1-7_workday",header=0,index=0,encoding='utf-8',columns=['time','name','inflow','outflow'])


# ## 线路
# # 提取线路需要先将date转化成相同的区间
# #读取静态数据
staticPath = 'F:\\datasets\\stationFlow\\subway_zdbm_station_new'
